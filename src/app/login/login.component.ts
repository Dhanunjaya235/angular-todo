...
export class LoginComponent implements OnInit {
  ...
  async onSubmit(data?: any) {
    await this.apiWrapper
      .POST(`/users/login`, data ? data : this.loginForm.value)
      .then((response) => {
        if (response.data.provider === GOOGLE_PROVIDER) {
          if (data) {
            this.initializeUser(response);
          } else {
            this.toaster.warningToaster(
              'User Registered Using Google. Sign in with Google to continue'
            );
          }
      ...
      .catch((error) => {
        ...
        this.toaster.errorToaster(
          'User Not Registered. Register To Use Application'
        );
      });
  }
